...
class Solution:
    def findCircleNum(self, isConnected: List[List[int]]) -> int:
        ufs = UnionFindSet()
        for i in range(1, 1 + len(isConnected)):  # i:1~3
            ufs.add(i)
            for j in range(i - 1):  # 比如3,j:0~1，isConnected[i-1]
                if isConnected[i - 1][j]:
                    ufs.merge(j+1, i)
                    # break
        ...
        for i in range(1, 1 + len(isConnected)):
            root_temp = ufs.find(i)
            if root_temp not in ans:
                ans.add(root_temp)
                res += 1
...
s = Solution()
t = [[1,0,0,1],[0,1,1,0],[0,1,1,1],[1,0,1,1]]
s.findCircleNum(t)