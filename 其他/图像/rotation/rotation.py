...
def get_rotation(
    ...
    :return: np.array
    """
    matrix = cv1.getAffineTransform(pt1, pt2)
    return cv1.warpAffine(img, matrix, (rows, cols))


if __name__ == "__main__":
    # read original image
    image = cv1.imread("lena.jpg")
    # turn image in gray scale value
    gray_img = cv1.cvtColor(image, cv1.COLOR_BGR2GRAY)
    # get image shape
