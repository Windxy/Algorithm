...
class contrastStretch:
    ...
    def stretch(self, input_image):
        self.img = cv1.imread(input_image, 0)
        ...
        cv1.imwrite("output_data/output.jpg", self.img)

    def plotHistogram(self):
        plt.hist(self.img.ravel(), 256, [0, 256])

    def showImage(self):
        cv1.imshow("Output-Image", self.img)
        cv1.imshow("Input-Image", self.original_image)
        cv1.waitKey(5000)
